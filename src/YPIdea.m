...
-(id) initWithValues:(NSDictionary*)dict
{
    ...
    id topics_dict = dict[@"topics"];
        if([topics_dict isKindOfClass:[NSArray class]])
        {
            NSMutableArray * objs = [[NSMutableArray alloc] initWithCapacity:[(NSArray*)topics_dict count]];

            if([(NSArray*)topics_dict count] > 0)
            {
            	YPTopic* d;
            	
                for (NSDictionary* dict in (NSArray*)topics_dict)
                {
                    if([dict isKindOfClass:[NSString class]])
                    {
                    	d = [[YPTopic alloc] initWithObjectId:(NSString*)dict];
                    }
                    else
                    {
                    	d = [[YPTopic alloc] initWithValues:dict];
                    }
                    
                    [objs addObject:d];
                }

                _topics = [[NSArray alloc] initWithArray:objs];
            }
            else
            {
...
-(NSDictionary*) asDictionary
{
    ...
    if(_defaultfrequency != nil) dict[@"defaultfrequency"] = _defaultfrequency ;
        ...
        if(_topics != nil)
    {
        if([_topics isKindOfClass:[NSArray class]])
        {
            NSMutableArray * array = [[NSMutableArray alloc] init];
            for( YPTopic *topics in (NSArray*)_topics)
            {
                [array addObject:[(YPObject*)topics asDictionary]];
            }

            dict[@"topics"] = array;
        }
        else if(_topics && [_topics isKindOfClass:[YPDate class]])
        {
            NSString * dateString = [(YPDate*)_topics toString];
            if(dateString)
            {
                dict[@"topics"] = dateString;
            }
        }
        else
        {
        if(_topics != nil) dict[@"topics"] = [(YPObject*)_topics asDictionary];
        }
    }
